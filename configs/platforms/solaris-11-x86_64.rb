platform "solaris-11-x86_64" do |plat|
...
basedir=default" > /var/tmp/vanagon-noask]
  plat.provision_with(%[echo "mirror=https://artifactory.delivery.puppetlabs.net/artifactory/generic__remote_opencsw_mirror/testing" > /var/tmp/vanagon-pkgutil.conf])
  plat.provision_with(%[pkgadd -n -a /var/tmp/vanagon-noask -d http://get.opencsw.org/now all])
  plat.provision_with(%[/opt/csw/bin/pkgutil --config=/var/tmp/vanagon-pkgutil.conf -y -i libffi_dev autoconf gcc4core CSWxz-5.2.8,REV=2022.11.16])
  plat.provision_with(%[ntpdate pool.ntp.org])
